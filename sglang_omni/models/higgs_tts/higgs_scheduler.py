...
"""Higgs TTS stage-facing scheduler.

Finish semantics: the multi-codebook sampler's ``generation_done`` flag (set
after EOC wind-down) is authoritative; ``max_new_tokens`` is a defensive cap.
"""
    ...
    def select_requests(
        ...
        for i, req in enumerate(schedule_batch.reqs):
            ...
            if sched_req is None:
                logger.warning("Higgs SGLang req %s not found in req_id_map", req.rid)
                continue
...
class HiggsIterationController:
    ...
    def update_request(
        self, request: SchedulerRequest, output_token_id: Any | None
    ) -> None:
        data = request.data
        req = data.req
        ...
        if output_token_id is not None and isinstance(output_token_id, int):
            if output_token_id >= 0:
    ...
    def is_finished(
        ...
        if data.req.is_chunked > 0:
            return False
        if getattr(data, "generation_done", False):
            return True
...
class HiggsScheduler:
    ...
    def __init__(
        ...
        *,
        ...
        self.resource_manager = HiggsResourceManager(
            token_to_kv_pool_allocator,
            req_to_token_pool,
            tree_cache,
            model=model,
        )
        self.iteration_controller = HiggsIterationController(
    ...
    def update(
        ...
        for request in scheduler_output.requests:
            ...
            if output is None:
                logger.warning(
                    "Missing Higgs output for request_id=%s", request.request_id
                )
                continue
        ...
        self.outbox.put(
            OutgoingMessage(
                ...
                metadata={
                    ...
                    "request_params": dict(payload.request.params or {}),
                },
            )
        )
        data.latest_stream_code_chunk = None
...
__all__ = [
    "HiggsBatchPlanner",
    "HiggsIterationController",
    "HiggsResourceManager",
    "HiggsScheduler",
]
